...
class UI():
    ...
    def processKeydown(self, event):
        ...
        # Set shader to texture and light shader, no reflection.
        if event.key == K_t:
            self.ge.shadernumber = 1

        # Set shader to reflection shader, no lighting.
        if event.key == K_y:
            self.ge.shadernumber = 2

        # Set shader to combination of reflection and lighting.
        if event.key == K_u:
            self.ge.shadernumber = 3
