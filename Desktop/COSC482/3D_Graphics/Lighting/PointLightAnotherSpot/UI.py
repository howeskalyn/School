...
class UI():
    ...
    def processKeydown(self, event):
        ...
        if event.key == K_r:
            self.ge.light.attenuation.x = 1
            self.ge.light.attenuation.y = 0
            self.ge.light.attenuation.z = 0
    ...
    def processKeyStates(self):
        ...
        if key[K_LALT] or key[K_RALT] or key[K_x]:
            ...
            self.StateProcessingSpotDirectionCamera()
        elif key[K_t]:
            self.StateProcessingLightAttenuation()
        elif self.ge.cameranum == 0:
            self.StateProcessingSphericalCamera()
        else:
            self.StateProcessingYPRCamera()

    # Alter the spot light attenuation values.
    def StateProcessingLightAttenuation(self):
        key = pygame.key.get_pressed()
        attC = self.ge.light.attenuation.x
        attL = self.ge.light.attenuation.y
        attQ = self.ge.light.attenuation.z
        if key[K_y]:
            if key[K_UP]:
                attC += 0.01
            if key[K_DOWN]:
        ...
        if attQ < 0:
            attQ = 0
        self.ge.light.attenuation.x = attC
        self.ge.light.attenuation.y = attL
        self.ge.light.attenuation.z = attQ

    # Alter the spot light direction, cutoff, and exponent.
    def StateProcessingSpotDirectionCamera(self):
        key = pygame.key.get_pressed()
        # Process a change to the spot cutoff.
        if key[K_h]:
            # Increase the spot cutoff.
            if key[K_UP]:
                cutoff = self.ge.light.spotCutoff + 0.5
                if cutoff < 0:
                    cutoff = 0
                if cutoff > 180:
                    cutoff = 180
                self.ge.light.spotCutoff = cutoff

            # Decrease the spot cutoff.
            if key[K_DOWN]:
                cutoff = self.ge.light.spotCutoff - 0.5
                if cutoff < 0:
                    cutoff = 0
            ...
            # Increase the spot exponent.
            if key[K_UP]:
                spexp = self.ge.light.spotExponent + 0.5
                if spexp < 0:
                    spexp = 0
                if spexp > 200:
                    spexp = 200
                self.ge.light.spotExponent = spexp

            # Decrease the spot exponent.
            if key[K_DOWN]:
                spexp = self.ge.light.spotExponent - 0.5
                if spexp < 0:
                    spexp = 0
            ...
            if key[K_UP]:
                self.ge.spotdirectioncamera.addPsi(1)
                self.ge.setViewMatrix()
            ...
            if key[K_LEFT]:
                self.ge.spotdirectioncamera.addTheta(1)
                self.ge.setViewMatrix()
