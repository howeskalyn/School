...
class UI():
    ...
    def processEvents(self, event):
        ...
        # Drag and drop load of the object file.
        if event.type == DROPFILE:
            dirpath = os.path.dirname(event.file)
            filename = os.path.basename(event.file)
            totaldir = os.path.join(dirpath, '')
            extension = os.path.splitext(filename)[1].lower()
            if extension == ".obj":
                self.ge.loadModel(totaldir, filename)
    ...
    def processKeydown(self, event):
        ...
        if event.key == K_0:
            self.ge.backgroundnum = 0

        if event.key == K_1:
            self.ge.backgroundnum = 1
