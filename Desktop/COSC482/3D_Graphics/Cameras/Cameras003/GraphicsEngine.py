...
class GraphicsEngine():
    ...
    def __init__(self):
        ...
        self.PVMLoc = glGetUniformLocation(self.shaderProgram, "PVM")
    ...
    def update(self):
        ...
        if self.showaxes:
            self.modelMatrix = glm.scale(glm.vec3(10, 10, 10))
            self.laodPVM()
            self.axes.draw()
            ...
            for i in range(-10, 11, 4):
                for j in range(-10, 11, 4):
                    for k in range(-10, 11, 4):
                        self.modelMatrix = glm.translate(glm.vec3(i, j, k))
                        self.laodPVM()
                        self.cube.draw()
        elif self.displayobjmode == 2:
            self.modelMatrix = glm.value_ptr(glm.mat4(1))
            self.laodPVM()
            self.cube.draw()
    ...
    def laodPVM(self):
        PVM = self.projectionMatrix * self.viewMatrix * self.modelMatrix
        glUniformMatrix4fv(self.PVMLoc, 1, GL_FALSE, glm.value_ptr(PVM))
