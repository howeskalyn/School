...
class GraphicsEngine():
    ...
    def update(self):
        ...
        if self.displayobjmode == 1:
            ...
            glUniformMatrix4fv(self.modelLoc, 1, GL_FALSE, glm.value_ptr(glm.mat4(1)))
            self.cube.draw()
    ...
    def setViewMatrix(self):
        if self.cameranum == 0:
            glUniformMatrix4fv(self.viewLoc, 1, GL_FALSE, glm.value_ptr(self.sphericalcamera.lookAt()))
        else:
            glUniformMatrix4fv(self.viewLoc, 1, GL_FALSE, glm.value_ptr(self.yprcamera.lookAt()))
