...
class Plane():
    # Constructor - takes in which axis will be 0
    def __init__(self, w=1, h=1, lon=20, lat=20, a = 'z'):
        self.w = w
        self.h = h
        self.lon = lon
        self.lat = lat
    ...
    def LoadDataToGraphicsCard(self):
        ...
        for i in range(self.lon + 1):
            for j in range(self.lat + 1):
                # Calculate vertices. (dependent on which axis is meant to be 0)
                if (self.axis == 'z'):
