...
def constructTriangles(n):
    ...
    while i < n:
        ...
        retData.append(outerPoints[i]) # appending vertex: RED, outerPoints[x], outerPoints[y]
        retData.append(innerPoints[i])

        retData.append(center)
        retData.append(outerPoints[i])
        retData.append(innerPoints[i - 1]) # using previous inner point, when i = 0, i = last index
...
class GraphicsEngine():
    ...
    def __init__(self):
        ...
        self.numPoints = 5
        self.data = constructTriangles(self.numPoints)
        self.loadStarData(self.numPoints)
    ...
    def loadStarData(self, numPoints):
        self.data = constructTriangles(numPoints)
