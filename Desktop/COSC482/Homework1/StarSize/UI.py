    ...
    def processEvent(self, event):
        ...
        if event.type == KEYDOWN:
            ...
            if event.key in [K_2, K_3, K_4, K_5, K_6, K_7, K_8, K_9]:
                ...
                self.ge.loadStarData(numPoints, self.ge.color, self.ge.length)

            # Increase number of points on star with up arrow key
            if event.key == K_UP:
                if self.ge.numPoints < 100:
                    self.ge.numPoints = self.ge.numPoints + 1
                self.ge.loadStarData(self.ge.numPoints, self.ge.color, self.ge.length)
            ...
            # increase star size with right arrow key
            if event.key == K_RIGHT:
                if self.ge.length < 1:
                    self.ge.length += 0.1
                self.ge.loadStarData(self.ge.numPoints, self.ge.color, self.ge.length)

            # decrease star size with left arrow key
            if event.key == K_LEFT:
                if self.ge.length > 0:
                    self.ge.length -= 0.1
                self.ge.loadStarData(self.ge.numPoints, self.ge.color, self.ge.length)
